// Sample use:
// node ping-thing-client-atlas.mjs >> ping-thing-atlas.log 2>&1 &
  ...
  for (let i = 0; ; ++i) {
    ...
    try {
      try {
        ...
        txStart = Date.now();
        const sendTxResult = await atlasRpcConnection.sendRawTransaction(
          tx.serialize(),
          {
            skipPreflight: true,
            maxRetries: 0,
        ...
        while (!confirmedTransaction) {
          ...
          await atlasRpcConnection.sendRawTransaction(tx.serialize(), {
            skipPreflight: true,
            maxRetries: 0,
          });
        }
...
await Promise.all([
  ...
  watchSlotSent(gSlotSent, rpcConnection),
  pingThing(),
]);
