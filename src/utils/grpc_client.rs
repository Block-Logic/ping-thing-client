...
/// Creates a subscription request for block updates
pub fn create_blocks_subscription_request(commitment: CommitmentLevel) -> SubscribeRequest {
    debug!(
        "[gRPC Client] Creating blocks subscription request with commitment: {:?}",
        commitment
    );
    ...
    debug!("[gRPC Client] Blocks subscription request created");
    request
}

/// Creates a subscription request for slot updates
pub fn create_slots_subscription_request(commitment: CommitmentLevel) -> SubscribeRequest {
    debug!(
        "[gRPC Client] Creating slots subscription request with commitment: {:?}",
        commitment
    );
    ...
    debug!("[gRPC Client] Slots subscription request created");
    request
}

/// Creates a subscription request for transaction updates
pub fn create_transaction_subscription_request(
    signature: String,
    commitment: CommitmentLevel,
) -> SubscribeRequest {
    debug!("[gRPC Client] Creating transaction subscription request for signature: {}, commitment: {:?}", signature, commitment);
    ...
    debug!(
        "[gRPC Client] Transaction subscription request created for signature: {}",
        signature
    );
    request
}
